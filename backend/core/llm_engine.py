...
class LLMEngine:
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 512) -> str:
        """Call the available LLM (Local GGUF -> Ollama -> Demo)."""
        if self.demo_mode:
    ...
    def index_document(self, doc_id: str, text: str, metadata: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        self._chunks_cache = None # Invalidate cache
...
You are NyayNeti, an Indian legal expert. Answer the question based on the provided context.
Cite specific cases and sections. If context is empty, say no documents are uploaded.<|eot_id|>
...
<|start_header_id|>assistant<|end_header_id|>"""
        ...
        return {
            ...
            "backend": "local-gguf" if self._llm else ("ollama" if self._check_ollama() else "demo")
        }
    ...
    def summarize_document(self, doc_id: str) -> Dict[str, Any]:
        ...
        text = "\n\n".join(ch.text for ch in chunks[:3]) # Summarize first 3 chunks
        prompt = f"""<|begin_of_text|><|start_header_id|>system<|end_header_id|>
Summarize this legal document concisely. Highlight parties, issue, and decision.<|eot_id|>
<|start_header_id|>user<|end_header_id|>
{text}<|eot_id|>
<|start_header_id|>assistant<|end_header_id|>"""
    ...
    def _generate_demo_response(self, prompt: str) -> str:
        if "bail" in prompt.lower():
            return "Bail is a matter of right in bailable offences and a matter of discretion in non-bailable offences. Key factors: nature of crime, antecedents, and flight risk. (Ref: State of Rajasthan v. Balchand)"
        return "I am currently in demo mode. Please ensure llama-cpp-python or Ollama is configured for real-time legal analysis."
